...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Parse or capture Codex `/status` usage percentages.")
    ...
    capture_command = commands.add_parser("capture", help="Launch Codex and control the desktop to capture `/status`.")
    capture_command.add_argument("--working-directory", type=Path, default=Path.cwd())
    capture_command.add_argument("--output-dir", type=Path, default=Path("output"))
    capture_command.add_argument("--launch-seconds", type=float, default=3.0)
    capture_command.add_argument("--status-seconds", type=float, default=2.0)
    capture_command.add_argument("--crop", nargs=4, type=float, metavar=("LEFT", "TOP", "RIGHT", "BOTTOM"), default=DEFAULT_RATIOS)
    capture_command.add_argument("--close-session", action="store_true")
    capture_command.add_argument("--force", action="store_true", help="Replace an existing reviewed capture.")
    return parser
