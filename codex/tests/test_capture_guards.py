...
import sys
import pytest
from codex_usage_grabber import capture
...
def test_existing_capture_stops_before_desktop_actions(tmp_path,monkeypatch):
    (tmp_path/'codex_status_window.png').write_bytes(b'preserved fixture')
    ...
    with pytest.raises(FileExistsError):capture.capture_usage(tmp_path,tmp_path)
    assert (tmp_path/'codex_status_window.png').read_bytes()==b'preserved fixture'
